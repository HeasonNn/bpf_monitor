...
int tc_dnat_func(struct __sk_buff *skb) {
  ...
  if (dnat_value) {
    if (protocol == IPPROTO_TCP) {
      bpf_skb_store_bytes(skb, IP_DST_OFF, &dnat_value->ip,
                          sizeof(dnat_value->ip), BPF_F_RECOMPUTE_CSUM);
      bpf_l3_csum_replace(skb, IP_CSUM_OFF, old_dst_ip, dnat_value->ip,
                          sizeof(dnat_value->ip));

      bpf_skb_store_bytes(skb, TCP_DST_OFF, &dnat_value->port,
                          sizeof(dnat_value->port), BPF_F_RECOMPUTE_CSUM);
      bpf_l4_csum_replace(skb, TCP_CSUM_OFF, old_dst_ip, dnat_value->ip,
                          sizeof(dnat_value->ip) | BPF_F_PSEUDO_HDR);
      bpf_l4_csum_replace(skb, TCP_CSUM_OFF, old_dst_port, dnat_value->port,
                          sizeof(dnat_value->port));
    }
    action = DNAT;
  }
...
int tc_snat_func(struct __sk_buff *skb) {
  ...
  if (snat_value) {
    if (protocol == IPPROTO_TCP) {
      bpf_skb_store_bytes(skb, IP_SRC_OFF, &snat_value->ip,
                          sizeof(snat_value->ip), BPF_F_RECOMPUTE_CSUM);
      bpf_l3_csum_replace(skb, IP_CSUM_OFF, old_src_ip, snat_value->ip,
                          sizeof(snat_value->ip));

      bpf_skb_store_bytes(skb, TCP_SRC_OFF, &snat_value->port,
                          sizeof(snat_value->port), BPF_F_RECOMPUTE_CSUM);
      bpf_l4_csum_replace(skb, TCP_CSUM_OFF, old_src_ip, snat_value->ip,
                          sizeof(snat_value->ip) | BPF_F_PSEUDO_HDR);
      bpf_l4_csum_replace(skb, TCP_CSUM_OFF, old_src_ip, snat_value->port,
                          sizeof(snat_value->port));
    }
    action = SNAT;
  } else {
